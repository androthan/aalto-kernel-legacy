...
extern struct omap_board_mux *aalto_r05_mux_ptr;
#endif
#else
...
static void aalto_wifi_init(void)
{
	...
	ret = gpio_request(AALTO_WIFI_PMENA_GPIO, "wifi_pmena");
	if (ret < 0) {
		printk(KERN_ERR "%s: can't reserve GPIO: %d\n", __func__,
			AALTO_WIFI_PMENA_GPIO);
		gpio_free(AALTO_WIFI_PMENA_GPIO);
}
	gpio_direction_output(AALTO_WIFI_PMENA_GPIO, 0);

	if (wl12xx_set_platform_data(&aalto_wlan_data))
	pr_err("WL12XX-DBG: error while setting wl12xx data\n");
	printk("WL12XX-DBG: Wi-Fi init done\n");
...
// Initializing WiFi H/W
device_initcall(aalto_wifi_init);
...
static void __init get_board_hw_rev(void)
{
	...
	switch (hw_revision) {
	case 0x0:
		printk("   Aalto HW Revision : REV 0.1 \n");
		break;
	case 0x1:
		printk("   Aalto HW Revision : REV 0.8 \n");
		break;
	case 0x2:
		printk("   Aalto HW Revision : REV 0.9 \n");
		break;
	case 0x3:
		printk("   Aalto HW Revision : REV 0.2 (2011-05-21) \n");
		break;
	case 0x4:
		printk("   Aalto HW Revision : REV 0.3 (2011-06-18) \n");
		break;
	case 0x5:
		printk("   Aalto HW Revision : REV 0.4 (2011-07-08) \n");
		break;
	case 0x6:
		printk("   Aalto HW Revision : REV 0.5 (2011-07-20) \n");
		break;
	case 0x7:
		printk("   Aalto HW Revision : REV 0.6 (2011-08-10) \n");
		break;
	case 0x8:
		printk("   Aalto HW Revision : REV 0.7 (2011-09-05) \n");
		break;
	case 0x9:
		printk("   Aalto HW Revision : REV 0.8 (2011-09-21) \n");
		break;
	default:
		printk("   Aalto HW Revision : UNKNOWN \n");
		break;
	}
}
...
#if (CONFIG_SAMSUNG_REL_HW_REV == 5)
	if(hw_revision == 0x06) {
		printk("aalto_r05_mux\n");
		omap3_mux_init(aalto_r05_mux_ptr, OMAP_PACKAGE_CBP);
	}
#endif
...
#else
	mi->bank[1].start = 0x90000000;
	mi->bank[1].size = 128 * SZ_1M;	/* DDR_CS1 128MB */
	mi->bank[1].node = 0;
#endif
