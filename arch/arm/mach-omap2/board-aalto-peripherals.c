...
/**
 * File Name : board-aalto-peripherals.c
 *
 * File Description :
 *
...
#if ( defined( CONFIG_MACH_SAMSUNG_AALTO ) && ( CONFIG_SAMSUNG_REL_HW_REV >= 1 ) )
...
static struct regulator_consumer_supply omap_board_vdda_dac_supply[] = {
	...
	REGULATOR_SUPPLY("vdda_dac1", NULL),
...
static struct __initdata twl4030_power_data aalto_t2scripts_data;
...
//	omap_board_vsim_supply.dev = mmc[0].dev;
	omap_board_vsim_supply.dev = mmc[2].dev;

	/* 
	 * AALTO(taejin.hyeon) 2011.03.18. 
	 * it is the temp. code for avoid crush with TSP
	 */     
//	omap_board_vmmc2_supply.dev = mmc[1].dev;
	omap_board_vmmc2_supply[0].dev = mmc[0].dev;
...
#define AALTO_GPIO_KEY_DEBOUNCE_INTERVAL 10

static struct gpio_keys_button aalto_gpio_keys_button[] = {
	/*
	{
		.desc			= "PWR_KEY",
		.type			= EV_KEY,
		.code			= KEY_POWER,
		.gpio			= OMAP_GPIO_KEY_AALTO_PWRON,
		.active_low		= 1, 
		.debounce_interval	= AALTO_GPIO_KEY_DEBOUNCE_INTERVAL,
	},
	{
		.desc			= "HOME_KEY",
		.type			= EV_KEY,
		.code			= KEY_HOME,
		.gpio			= OMAP_GPIO_KEY_AALTO_HOME,
		.active_low		= 1, 
		.debounce_interval	= AALTO_GPIO_KEY_DEBOUNCE_INTERVAL,
	},
	*/
	{
		...
		.type			= EV_KEY,
		.code			= KEY_VOLUMEUP,
		.gpio			= OMAP_GPIO_KEY_AALTO_VOLUP,
		.active_low		= 1, 
		.debounce_interval	= AALTO_GPIO_KEY_DEBOUNCE_INTERVAL,
	},
	{
		...
		.type			= EV_KEY,
		.code			= KEY_VOLUMEDOWN,
		.gpio			= OMAP_GPIO_KEY_AALTO_VOLDN,
		.active_low		= 1, 
		.debounce_interval	= AALTO_GPIO_KEY_DEBOUNCE_INTERVAL,
	},
	/*
...
static struct gpio_keys_platform_data aalto_gpio_keys_platform_data = {
	.buttons = aalto_gpio_keys_button,
	.nbuttons = ARRAY_SIZE(aalto_gpio_keys_button),
	//.rep = 1;
};

static struct platform_device aalto_gpio_key_device = {
	.name = "sec_key",
	.id = -1,
	.dev = {
		.platform_data = &aalto_gpio_keys_platform_data,
	},
};
#endif
...
// +LYG-AALTO	// For cam driver 15.04.2011 (jinokang)
/*	{
...
#endif
#if (CONFIG_SAMSUNG_REL_HW_REV < 2)	/* Aalto Battery Detect available for Rev0.2 or higher */
	samsung_charger_resources[0].start = -1;
#else
